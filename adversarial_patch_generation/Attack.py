...
parser.add_argument('--train_size', type=int, default=20, help="number of training images")
parser.add_argument('--test_size', type=int, default=10, help="number of test images")
...
parser.add_argument('--target', type=int, default=2, help="target label")
...
train_loader, test_loader = dataloader(args.train_size, args.test_size, args.batch_size, args.num_workers, 50000)

# Test the accuracy of model on trainset and testset
trainset_acc= test(model, train_loader)
...
for epoch in range(args.epochs):
    ...
    for (image, label) in train_loader:
        ...
        if predicted[0] != label and predicted[0].data.cpu().numpy() != args.target:
             ...
             # print("train_actual_total value is:", train_actual_total)
             applied_patch, mask, x_location, y_location = mask_generation(args.patch_type, patch, image_size=(3, 224, 224))
             # print("applied patch size is:", applied_patch.shape)
             # print("patch x location is:", x_location)
             # print("patch y location is:", y_location)
             ...
             # print("applied patch size is:", applied_patch.shape)
             plt.imshow(np.squeeze(np.transpose(perturbated_image, (2, 3, 1, 0))))
             plt.savefig("perturbated_image.png")
...
test_success_rate = test_patch_ig(args.patch_type, args.target, patch, test_loader, model, x_location, y_location)
print("Epoch:{} Patch attack success rate on testset with last epoch: {:.3f}%".format(epoch, 100 * test_success_rate))
